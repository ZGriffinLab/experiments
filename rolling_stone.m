...
%
%created heavily referencing EyelinkPicture.m, the METS program, and Calzone
%
...
try
    ...
    key = sprintf('%sy', 'f19');
    audStimsMap(key) = YNeutStimMap(neutStims{1,19});
    key = sprintf('%sn', 'f19');
    audStimsMap(key) = NNeutStimMap(neutStims{1,i});
    ...
    expData.add('response time');
    expData.add('vkok');
    expData.add('wav file duration');
    expData.add('');
    expData.nextRow();
    ...
    %so participant names cannot exceed 6 characters
    fileName = ['RS' participantId '.edf'];
    ...
    while(testTrials <= length(testStims))
        ...
        trialId = [ participantId '_practice_' testStims{1, testTrials} ];
        EyelinkSetTrialId(trialId);
        
        % This supplies the title at the bottom of the eyetracker display
        Eyelink('command', 'record_status_message "TRIAL %d/%d  %s"', testTrials, 6, testStims{1, testTrials}); 
        
        %ready eyelink for commands and clear eyelink screen
        ...
        %if you do not place this before it starts recording, then it will
        %not show up for play back animation
        ...
        stimandOrder.add(num2str(responseTime));
        stimandOrder.add(trigger);
        stimandOrder.add(num2str(wavDuration));
        stimandOrder.add('');
        stimandOrder.nextRow();
        
        %add trial variables to the edf file for the index and the image
        %used
        WaitSecs(0.001);
    ...
    while (currentTrial <= length(stimOrder))
        ...
        % This supplies the title at the bottom of the eyetracker display
        Eyelink('command', 'record_status_message "EXP TRIAL %d/%d  %s"', currentTrial, 90, stimOrder{1, currentTrial}); 
        ...
        bmpfile = sprintf('stimuli/visual/%sl.bmp', stimOrder{1, currentTrial}(1:3));
        iafile = sprintf('stimuli/IAsets/%sl.ias', stimOrder{1, currentTrial}(1:3)); 
        else
            imgfile = sprintf('stimuli/visual/%sr.jpg', stimOrder{1, currentTrial}(1:3));
            bmpfile = sprintf('stimuli/visual/%sr.bmp', stimOrder{1, currentTrial}(1:3));
            iafile = sprintf('stimuli/IAsets/%sr.ias', stimOrder{1, currentTrial}(1:3));
        end
        Eyelink('Message', '!V IAREA FILE %s', iafile); 
         else
             imgfile = sprintf('stimuli/visual/%s.jpg', stimOrder{1, currentTrial}(1:3));
             bmpfile = sprintf('stimuli/visual/%s.bmp', stimOrder{1, currentTrial}(1:3));
        end
        ...
        if (start == 'c')
            ...
            expData.add(num2str(responseTime));
            expData.add(trigger);
            expData.add(num2str(wavDuration));
            expData.add('');
            expData.nextRow();    
        end
